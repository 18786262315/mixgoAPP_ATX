...
class apk_install(unittest.TestCase, BasePage):
    ...
    def test_01_install_apk(self):
        ...
        tanchuang = self.d(className="android.widget.LinearLayout", instance=1).wait(timeout=2)
        
        if tanchuang:
            # 判断是否有权限获取
            number1 = self.d(resourceId="com.android.packageinstaller:id/current_page_text").get_text()
            f = re.compile(r"\d+")
